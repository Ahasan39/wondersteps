...
export interface AudioPreferences { version:1; musicEnabled:boolean; sfxEnabled:boolean; voiceEnabled:boolean }
// Voice starts only after Start, so first-time guidance is useful without page-load autoplay.
export const defaultAudioPreferences:AudioPreferences = {version:1,musicEnabled:false,sfxEnabled:false,voiceEnabled:true}
...
export function loadAudioPreferences(storage:PreferenceStorage):AudioPreferences {
 try {
  ...
  return {version:1,musicEnabled:'musicEnabled' in value&&value.musicEnabled===true,sfxEnabled:'sfxEnabled' in value&&value.sfxEnabled===true,voiceEnabled:!('voiceEnabled' in value)||value.voiceEnabled===true}
