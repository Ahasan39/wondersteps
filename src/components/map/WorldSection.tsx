...
export function LevelNode({ level, status }: { level: Level; status: LevelStatus }) {
  return <div className={`level-stop stop-${(level.id - 1) % 5} ${status}`}>
    {status === 'locked'
      ? <button className="level-node" disabled title="Available in a future part of your adventure" aria-label={`Level ${level.id}: ${level.name}, locked`}><LockKeyhole size={20}/><span>{level.id}</span></button>
    ...
    {status === 'current' && <span className="start-label">Play · start here</span>}
    {status === 'completed' && <span className="completed-label">Completed</span>}
  </div>
}
...
export function WorldSection({ world, levels, index }: { world: World; levels: Level[]; index: number }) {
  ...
  return <section className={`world-section ${world.theme}`} aria-labelledby={`world-${world.id}`}>
    ...
    <header className="world-heading">
      ...
      {index > 0 && <span className="world-lock"><LockKeyhole size={14}/> A future part of your journey</span>}
    </header>
    <div className="world-path"><LevelPath/>{levels.map(level => <LevelNode key={level.id} level={level} status={statusFor(level.id)}/>)}</div>
  </section>
}
