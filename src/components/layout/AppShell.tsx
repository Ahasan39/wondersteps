...
export function AppShell() {
  ...
  const playLevel=pathname.startsWith('/play/')?levels.find(level=>String(level.id)===pathname.split('/')[2]):undefined
  ...
  return <div className={pathname.startsWith('/play/') ? 'app-shell play-route' : 'app-shell'}>
    ...
    <header className="game-header">
      <Link to="/levels" className="gameplay-back" aria-label="Back to levels"><ArrowLeft size={20}/><span>Back</span></Link>
      <span className="gameplay-heading">LEVEL {playLevel?.id}</span>
      ...
      <div className="hud">
        ...
        <IconButton title="Mute or enable music, effects and voice" aria-label={soundEnabled ? 'Turn sound preference off' : 'Turn sound preference on'} aria-pressed={soundEnabled} onClick={toggleSound}>{soundEnabled ? <Volume2 size={21}/> : <VolumeX size={21}/>}</IconButton>
    ...
    <footer className="app-footer"><Footprints size={15}/><span>Little steps. Wonderful discoveries.</span><span className="footer-phase">World 1 game experience · Phase 4.1</span><span className="footer-credit">Design and Developed by <a href="https://ahasan39.github.io/"><strong>Ahasan39</strong></a></span></footer>
  </div>
}
