...
export function PreferencesProvider({children}:{children:ReactNode}){
 ...
 const toggleSound=useCallback(()=>commit(value=>{const enabled=!(value.musicEnabled||value.sfxEnabled||value.voiceEnabled);return {...value,musicEnabled:enabled,sfxEnabled:enabled,voiceEnabled:enabled}}),[commit])
 ...
 const toggleVoice=useCallback(()=>commit(value=>({...value,voiceEnabled:!value.voiceEnabled})),[commit])
 const {preferences,persisted}=state
 return <PreferencesContext.Provider value={{...preferences,soundEnabled:preferences.musicEnabled||preferences.sfxEnabled||preferences.voiceEnabled,toggleSound,toggleMusic,toggleSfx,toggleVoice,preferencesPersisted:persisted}}>{children}</PreferencesContext.Provider>
}
